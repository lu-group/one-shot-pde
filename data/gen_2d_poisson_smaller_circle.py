...
def load_all_data(Nx, Ny, dname, PATH, correction, isplot=False):
    data_T_grid = np.loadtxt(f"{PATH}/data_0.05/data_0/f_T_grid.txt",skiprows=9)
    ...
    np.savetxt(f"{PATH}/data_0.05/data_0/data_T_grid.txt", data_T_grid[inds])
    ...
    inds_b = [i for i in range(len(data_new)) 
              if ((data_new[i, 2] - hx - 0.5)**2 + (data_new[i, 3] - 0.5)**2 > (radius)**2)
              and ((data_new[i, 2] - 0.5)**2 + (data_new[i, 3] - hy - 0.5)**2 > (radius)**2)
              and ((data_new[i, 2] + hx - 0.5)**2 + (data_new[i, 3] - 0.5)**2 > (radius)**2)
              and ((data_new[i, 2] - 0.5)**2 + (data_new[i, 3] + hy - 0.5)**2 > (radius)**2)
              and ((data_new[i, 2] - 0.5)**2 + (data_new[i, 3] - 0.5)**2 > (radius)**2)
...
def generate_random_points(N_f, dname):
    ...
    geom2 = dde.geometry.Disk((0.5,0.5), 0.15)
...
def gen_datasets(sigma, num_func, parent_dir = "./", gen = True):
    ...
    if gen:
        for i in range(num_func):
            ...
            #generate_random_points(Nx*Ny, dname)
    ...
    # Then load the data files to FEM solver
        ...
        for i in range(num_func):
            ...
            load_all_data(Nx, Ny, dname, PATH, correction=False, isplot=False)
    return
...
if __name__ == "__main__":
    ...
    #gen_datasets(args.sigma, args.num, gen = True)
    gen_datasets(args.sigma, args.num, gen = False)
