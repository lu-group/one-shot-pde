...
def load_all_data(M, N, N_f, N_b, l, a, l_new, a_new, dname, gen = False, correction = False, grid = False, isplot = False):
    # Define the domain
    ...
    f_0 = np.loadtxt(f"{dname}/f_0_grid.dat")[:, 1]
    u_0 = np.loadtxt(f"{dname}/u_0_grid.dat")[:, 1]
    ...
    if not grid:    
        x_train = np.loadtxt(f"{dname}/u_new.dat")[:, 0].reshape((-1, 1))
        x_test = np.loadtxt(f"{dname}/u_new_grid.dat")[:, 0].reshape((-1, 1))
        ...
        if correction:
            ...
            u_init = np.loadtxt(f"{dname}/u_0_grid.dat")[:, 1].reshape((-1, 1))
            y_test = u_new - u_init
