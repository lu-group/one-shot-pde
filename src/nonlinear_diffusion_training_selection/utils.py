...
def gen_data_GRF_rf(M, Nx, Nt, l, a, dname, isplot = False):
    space = GRF(1, length_scale=l, N=M, interp="cubic")
    features = space.random(1)
    f_T = lambda x, t: a * space.eval_u(features, x).T + 0 * t
...
def gen_data_GRF_sin(M, Nx, Nt, l, a, dname, isplot = False):
...
def gen_data_GRF_cos(M, Nx, Nt, l, a, dname, isplot = False):
...
def gen_data_GRF_freq(M, Nx, Nt, l, a, dname, isplot = False):
    f0 = lambda x: np.sin(2 * np.pi * x *1.5)
    space = GRF(1, length_scale=l, N=M, interp="cubic")
    features = space.random(1)
    f_T = lambda x, t: f0(x) + a * space.eval_u(features, x).T + 0 * t
    x, t, u_T =  compute_numerical_solution(f_T,M, M)
