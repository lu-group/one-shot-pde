...
def fixed_point_iteration(model, dname, ftype, PATH, isplot=False):
    # new source function
    ...
    # plt.imshow(u_new, cmap = "rainbow",  origin='upper', extent=(0,1,0,1), aspect='auto')
    # plt.colorbar()
    # plt.xlabel("x")
    # plt.ylabel("t")
    # plt.show()
    ...
    # initial guess
    u = np.loadtxt(f"{PATH}/data_0.80/data_0/u_0_grid.dat")
    ...
    if isplot:
        ...
        plt.imshow(np.rot90(abs(u - u_new)), cmap = "rainbow", extent=(0,1,0,1), aspect='auto')
        plt.colorbar()
        plt.xlabel("x")
        plt.ylabel("t")
        ...
        plt.imshow(np.rot90(u), cmap = "rainbow",  origin='upper', extent=(0,1,0,1), aspect='auto')
        plt.colorbar()
        plt.xlabel("x")
        plt.ylabel("t")
        plt.savefig(f"{dname}/pred_FPI.png")
        plt.show()
        
        plt.figure()
        plt.rcParams.update({'font.size': 20,"savefig.dpi": 200, "figure.figsize": (8, 6)})
        plt.subplots_adjust(left=0.15, right=0.9, top=0.9, bottom=0.15)
        plt.imshow(np.rot90(u_new), cmap = "rainbow",  origin='upper', extent=(0,1,0,1), aspect='auto')
        plt.colorbar()
        plt.xlabel("x")
        plt.ylabel("t")
        plt.show()
...
def main(sigma, num_func, ftype, parent_dir = "../../data/", gen = False):
    ...
    for i in range(num_func):
        ...
        err = fixed_point_iteration(model, dname, ftype, PATH, isplot=False)
        errs.append(err)
    
    np.savetxt(os.path.join(f"{PATH}/data_{sigma}_{ftype}/", f"errs_FPI_{ftype}.dat"), errs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ftype", type=str, default="cos")
    args = parser.parse_args()
    
    print(f"Arguments received: num={args.num}, sigma={args.sigma}, ftype={args.ftype}")
    current_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"{LOG_DIR}/log_{args.sigma}_{args.ftype}_{current_time}.log"
