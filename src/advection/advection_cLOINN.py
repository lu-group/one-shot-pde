...
class UpdateOutput(Callback):
    ...
    def on_batch_begin(self):
        ...
        if self.model.train_state.epoch % 1000 == 0:
            err = np.linalg.norm(outputs[0::4] - self.u_new)/np.linalg.norm(self.u_new)
    ...
    def get_inputs(self):
        ...
        x_b = np.array([[[xt[0] - self.hx, xt[1] - self.ht]] for xt in self.x_train])
        x_r = np.array([[[xt[0], xt[1]- self.ht]] for xt in self.x_train])
    ...
    def construct_local_domain(self, f, outputs):
        outputs_u = outputs[0::4]
        u_l = outputs[1::4]
        u_b = outputs[2::4]
        u_r = outputs[3::4]
        inputs = np.concatenate((f, u_l, u_b, u_r), axis = 1)
...
def main():
    ...
    print(b_steps)
    #np.savetxt("data_{}/b_steps_FPC.dat".format(num_std),b_steps)
    np.savetxt("data_{}/errs_FPC.dat".format(num_std),errs)
