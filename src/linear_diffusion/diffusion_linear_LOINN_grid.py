...
def solve_nn(Nx, Nt, N_b, dataset_G, data, pre_layers, best_step, dname, isplot=False):
    ...
    losshistory, train_state = model.train(epochs=97000,disregard_previous_best=True,  callbacks=[update, checker], model_save_path = "model/lmodel")
    dde.saveplot(losshistory, train_state, issave=False, isplot=isplot, output_dir = f"{dname}/history_LOINN{sname}")
    model.restore("model/lmodel-100000.ckpt", verbose=1)
