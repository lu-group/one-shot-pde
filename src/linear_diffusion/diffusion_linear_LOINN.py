...
class UpdateOutput(Callback):
    def __init__(self, Nx, Nt, N_b, dataset, net, pre_layers, best_step, x_train, dname, grid):
        ...
        self.ht = 1/(Nt-1)
        if grid:
            f_new = np.loadtxt(f"{dname}/f_new_grid.dat")[:, 2]
        else:
            f_new = np.loadtxt(f"{dname}/f_new.dat")[:, 2]
...
def solve_nn(Nx, Nt, N_b, dataset_G, data, pre_layers, best_step, dname, grid, isplot=False):
    ...
    update = UpdateOutput(Nx, Nt, N_b, dataset_G, net, pre_layers, best_step, x_train, dname, grid)
...
def main(sigma, num_func, grid, parent_dir = "../../data/", gen = False):
    ...
    for i in range(num_func):
        ...
        err,b_step = solve_nn(Nx, Nt, N_b, dataset_G, dataset, pre_layers, best_step, dname, grid, False)
        #err,b_step = apply(solve_nn, (Nx, Nt, N_b, dataset_G, dataset, pre_layers, best_step, dname, grid, False))
        errs.append(err)
        b_steps[i][0] = b_step
