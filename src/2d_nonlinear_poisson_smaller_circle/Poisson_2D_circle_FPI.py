...
def fixed_point_iteration(Nx, Ny, model, dname, PATH, isplot=False):
    ...
    inds_c = [i for i in range(len(data_new_grid)) if ((data_new_grid[i, 2] - 0.5)**2 + (data_new_grid[i, 3] - 0.5)**2 <= radius**2) and i not in np.argwhere(np.isnan(data_new_grid))[:, 0]] # points on the circle 8
    ...
    inds_bcc = [i for i in inds_all if i not in inds_c and (
              not (((data_new_grid[i, 2] - hx - 0.5)**2 + (data_new_grid[i, 3] - 0.5)**2 > radius**2)
              and ((data_new_grid[i, 2] - 0.5)**2 + (data_new_grid[i, 3] - hy - 0.5)**2 > radius**2)
              and ((data_new_grid[i, 2] + hx - 0.5)**2 + (data_new_grid[i, 3] - 0.5)**2 > radius**2)
              and ((data_new_grid[i, 2] - 0.5)**2 + (data_new_grid[i, 3] + hy - 0.5)**2 > radius**2)))] # 108
    ...
    print(len(inds_nan), len(inds_all), len(inds_c), len(inds_bc), len(inds_bcc)) #716 9284 0 396 80
...
def main(sigma, num_func, parent_dir = "../../data/", gen = False):
    ...
    new_dir = "data_nonlinear_poisson_smaller_circle"
    ...
    for i in range(num_func):
        ...
        err = fixed_point_iteration(Nx, Ny, model, dname, PATH, isplot=False)
        errs.append(err)

    print(errs)
    np.savetxt(os.path.join(f"{PATH}/data_{sigma}/", "errs_FPI.dat"), errs)
