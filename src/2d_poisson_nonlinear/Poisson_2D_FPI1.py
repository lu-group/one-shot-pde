...
def construct_data(f, u):
    ...
    inputs = np.hstack((u[:-2, 1:-1].reshape((-1, 1)), u[1:-1, :-2].reshape((-1, 1)), u[2:, 1:-1].reshape((-1, 1)), u[1:-1, 2:].reshape((-1, 1)), f[1:-1, 1:-1].reshape((-1, 1))))
...
def main():
    ...
    dataset_G, dataset = gen_all_data(M, Nx, Ny, N_f, N_b, 0.01, 0.1, 0.1, 0.10, False, "1_0.05/data_0")
    best_step = "101090"
    pre_layers = [5, 64, 1]
    ...
    for i in range(num):
        ...
        os.makedirs("data1_{}/data_{}".format(std_num, i), exist_ok = True)
