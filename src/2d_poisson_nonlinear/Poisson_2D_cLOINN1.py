...
def solve_nn(Nx, Nt, dataset_G, data, pre_layers, best_step, d_num):
    ...
    l2_errs = []
    net = dde.nn.FNN([2] + [16] + [1], "tanh", "LeCun normal")#, "Glorot uniform")
    ...
    iters = 20000
    checker = dde.callbacks.ModelCheckpoint("model2/model.ckpt", save_better_only=True, period=5000)
...
def main():
    ...
    for i in range(num):
        ...
        os.makedirs("data1_{}/data_{}".format(num_std, i), exist_ok = True)
        d_num = "1_{}/data_{}".format(num_std, i)
        ...
        b_steps[i][0] = b_step
        print(b_steps)
        np.savetxt("data1_{}/b_steps_FPC.dat".format(num_std),b_steps)
    print(errs)
    print(b_steps)
    #np.savetxt("data_{}/b_steps_FPC.dat".format(num_std),b_steps)
    np.savetxt("data1_{}/errs_FPC.dat".format(num_std),errs)
